...
def scrape538(dateInfo):
    ...
    target_website = "https://projects.fivethirtyeight.com/"+str(season)+"-nfl-predictions/games/"
    browser = MyBot()
    ...
    # Locate the current week's picks
    weeks = body.find_all("section", class_="week")
    currWeek = None
    for week in weeks:
        # get the contents of the first h3
        titleh3 = week.find("h3")
        # check if it says "week 14" (for example)
        if titleh3.text == "Week "+str(currWeekNum):
            currWeek = week

    # Get an array of all instances of 'div.game'
    matchups = week.find_all("div", class_="game")
    # Iterate over them:
    for matchup in matchups:
        # Determine the teams playing
        # Determine the picked team
        # Determine the probability of the pick winning
        # Construct MatchupID
        # Save this data to the schedule
        matchupID = currWeek+"-"
